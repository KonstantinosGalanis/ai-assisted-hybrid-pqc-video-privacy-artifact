...
def _derive_seed_for_roi(ff, master_key: str, frame_idx: int, track_id: int, cls_id: int, frame_token: str,
                         bbox: Optional[Tuple[int, int, int, int]] = None,
                         mask: Optional[np.ndarray] = None, roi_index: int = -1) -> bytes:
    ...
    if hasattr(ff, 'derive_policy_seed') and hasattr(ff, 'policy_tag_for_class'):
        try:
            ...
            if 'frame_class' in sig.parameters:
                kwargs['frame_class'] = frame_token
            elif 'map_name' in sig.parameters:
                kwargs['map_name'] = frame_token
            if hasattr(ff, 'POLICY_SCOPE') and 'scope' in sig.parameters:
            ...
            if 'roi_index' in sig.parameters:
                kwargs['roi_index'] = int(roi_index)
    ...
    if hasattr(ff, 'derive_seed'):
        try:
            ...
            kwargs = {}
            if 'bbox' in params and bbox_tuple is not None:
                kwargs['bbox'] = bbox_tuple
            if 'mask_hash' in params and mask_hash is not None:
                kwargs['mask_hash'] = mask_hash
            if 'roi_index' in params:
                kwargs['roi_index'] = int(roi_index)
            return ff.derive_seed(master_key, int(frame_idx), int(track_id), int(cls_id), frame_token, **kwargs)
        except Exception:
            pass
        return ff.derive_seed(master_key, int(frame_idx), int(track_id), int(cls_id), frame_token)

    raise RuntimeError('framework module does not expose derive_seed/derive_policy_seed')

def _keystream_bytes_for_roi(ff, seed: bytes, frame_token: str, nbytes: int) -> bytes:
    stream_seed = seed
    if hasattr(ff, 'derive_stream_seed'):
        try:
            stream_seed = ff.derive_stream_seed(seed, 'nistroi')
        except Exception:
            stream_seed = seed
    if hasattr(ff, 'chacha20_stream_bytes'):
        ks = ff.chacha20_stream_bytes(stream_seed, str(frame_token), 'nistroi', int(nbytes))
        return np.asarray(ks, dtype=np.uint8).tobytes()
    ks = ff.keystream_u8(stream_seed, frame_token, nbytes)
    return np.asarray(ks, dtype=np.uint8).tobytes()

def encrypt_frames_with_sidecar(frames: List[np.ndarray], recs: List[dict], ff, master_key: str, force_map_name: Optional[str] = None, preview_mode: str = "chacha", force_frame_class: Optional[str] = None, framework_base_path: Optional[str] = None) -> List[np.ndarray]:
    out: List[np.ndarray] = []
    rec_by_idx = {int(r.get('frame_idx', i)): r for i, r in enumerate(recs)}
    base_ff = resolve_sidecar_module(ff, framework_base_path)
    for idx, fr in enumerate(frames):
        default_token = force_frame_class or force_map_name or 'UNK'
        rec = rec_by_idx.get(idx, {'frame_idx': idx, 'rois': [], 'frame_class': default_token})
        frame_token = force_frame_class or force_map_name or str(rec.get('frame_class', rec.get('map_name', 'UNK')))
        cur = fr.copy()
        h, w = cur.shape[:2]
        for r in rec.get('rois', []) or []:
            ...
            seed = _derive_seed_for_roi(base_ff, master_key, idx, track_id, cls_id, frame_token, bbox=(x1, y1, x2, y2), mask=mask, roi_index=int(r.get('roi_index', -1)))
            if hasattr(ff, 'apply_public_video_preview'):
                roi_out = ff.apply_public_video_preview(base_ff, roi, mask, seed, frame_token, preview_mode)
            else:
                roi_out = base_ff.encrypt_masked_bytes_cpu(roi, mask, seed, frame_token)
...
def derive_used_keystream_bytes(recs: List[dict], ff, master_key: str, force_map_name: Optional[str] = None) -> bytes:
    ...
    for rec in recs:
        ...
        frame_token = force_map_name or str(rec.get('frame_class', rec.get('map_name', 'UNK')))
        for r in rec.get('rois', []) or []:
            bbox = r.get('bbox')
            ...
            track_id = int(r.get('track_id', -1))
            cls_id = int(r.get('cls', -1))
            seed = _derive_seed_for_roi(core_ff, master_key, frame_idx, track_id, cls_id, frame_token, bbox=(x1, y1, x2, y2), mask=mask, roi_index=int(r.get('roi_index', -1)))
            chunks.append(_keystream_bytes_for_roi(core_ff, seed, frame_token, nbytes))
...
def resolve_sidecar_module(ff_or_path, framework_base_path: Optional[str] = None):
    ...
    if ff_path is not None:
        for name in (
            "framework_faster_chachapoly.py",
            "framework_faster_chachapoly (1).py",
            "framework_faster.py",
            "framework_succesful.py",
        ):
...
def framework_cli_encrypt(framework_path: str, in_path: str, out_path: str, master_key: str,
    ...
    if isinstance(video_preview_mode, str):
        vm = video_preview_mode.strip().lower()
        video_preview_mode = vm if vm in {"mosaic", "blur", "black", "chacha"} else "chacha"
    if isinstance(audio_preview_mode, str):
        am = audio_preview_mode.strip().lower()
        audio_preview_mode = am if am in {"chacha", "mute", "passthrough"} else "chacha"
...
def framework_cli_decrypt(framework_path: str, in_path: str, out_path: str, master_key: str,
                          ...
                          video_preview_mode: Optional[str] = None,
                          audio_preview_mode: Optional[str] = None) -> None:
    if isinstance(video_preview_mode, str):
        vm = video_preview_mode.strip().lower()
        video_preview_mode = vm if vm in {"mosaic", "blur", "black", "chacha"} else "chacha"
    if isinstance(audio_preview_mode, str):
