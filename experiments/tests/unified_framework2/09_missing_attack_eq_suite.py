...
def read_frames(path: str, max_frames: int = 25, stride: int = 1) -> Tuple[List[np.ndarray], float]:
    if is_video(path):
...
def _derive_seed_for_roi(
    ...
    """
    Derive the ROI seed using whichever naming convention the framework exposes.

    Chaos historically used map_name.
    ChaCha uses frame_class.
    """
    if hasattr(ff, "derive_policy_seed"):
        ...
        if "roi_index" in sig.parameters:
            # This test helper does not receive roi_index here, so omit unless needed.
            pass
...
def oracle_apply_frames(
    ...
    for frame_idx, frame in enumerate(out_frames):
        ...
        if rec is None:
            continue
        map_name = str(rec.get("frame_class", rec.get("map_name", "cubic")))
...
def main():
    ...
    plain_frames, _ = read_frames(args.plain, args.max_frames, args.stride)
    cipher_frames, _ = read_frames(args.cipher, args.max_frames, args.stride)
    ...
    recs = {int(k): v for k, v in recs.items() if int(k) < n}
