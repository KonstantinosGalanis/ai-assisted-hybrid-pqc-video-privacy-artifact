...
def main() -> None:
    ...
    ap.add_argument("--framework_faster_path", default=str(THIS_DIR / "framework_proxy_payload_matched_chachapoly_optimized.py"))
    ap.add_argument("--framework_base_path", default=str(THIS_DIR / "framework_faster_chachapoly_optimized.py"))
    ...
    ap.add_argument("--mask_ablation_preview_modes", default="chacha", help="Test 17 preview modes for payload/proxy framework")
    ...
    if wants(9):
        roi_for_test9 = choose_roi_sidecar_for_manifest(
            dataset_dir,
            assets.get("manifest"),
            assets.get("roi_sidecar"),
            notes,
        )

        if assets.get("plain") and assets.get("cipher") and roi_for_test9 and master_key:
            cmd = [
                ...
                "--plain", assets["plain"],
                "--cipher", assets["cipher"],
                "--roi_sidecar", roi_for_test9,
                "--max_frames", str(args.max_frames),
                "--out", str(out_dir / "09_attacks"),
            ]

            if assets.get("plain2"):
                cmd.extend(["--plain2", assets["plain2"]])
            if assets.get("cipher2"):
                cmd.extend(["--cipher2", assets["cipher2"]])
            if assets.get("plain_ref"):
                cmd.extend(["--plain_ref", assets["plain_ref"]])
            if assets.get("cipher_ref"):
                cmd.extend(["--cipher_ref", assets["cipher_ref"]])
            ...
            add_skip(
                master_report["tests"],
                "09_attacks_eq",
                "Need plain media, cipher media, ROI sidecar, and master key.",
            )
    ...
    if wants(14):
        ...
        if assets.get("cipher") and roi_for_test14 and assets.get("payload") and assets.get("manifest") and master_key:
            cmd = [
                sys.executable, "14_chacha_aead_tamper_response.py",
                "--cipher", assets["cipher"],
            ...
            if framework_base_path:
                cmd.extend(["--framework_base_path", framework_base_path])
            run_cmd("14_chacha_aead_tamper_response", cmd, master_report["tests"])
        else:
    ...
    if wants(15):
        ...
        if assets.get("cipher") and roi_for_test15 and assets.get("payload") and assets.get("manifest") and master_key:
            cmd = [
                sys.executable, "15_chacha_roi_replay_substitution.py",
                "--cipher", assets["cipher"],
            ...
            if framework_base_path:
                cmd.extend(["--framework_base_path", framework_base_path])
            run_cmd("15_chacha_roi_replay_substitution", cmd, master_report["tests"])
        else:
    ...
    if wants(16):
        if assets.get("roi_sidecar"):
            cmd = [
                sys.executable, "16_chacha_nonce_context_binding.py",
                "--roi_sidecar", assets["roi_sidecar"],
                "--out", str(out_dir / "16_chaos_nonce_context"),
            ]
            if assets.get("payload"):
                cmd.extend(["--payload", assets["payload"]])
            run_cmd("16_chacha_nonce_context_binding", cmd, master_report["tests"])
        else:
