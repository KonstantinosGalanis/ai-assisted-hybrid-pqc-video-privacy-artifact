...
"""
14_chacha_aead_tamper_response.py

ChaCha20-Poly1305 payload/security test for the current payload-aware framework.
It mutates one field at a time and verifies that decrypt/reconstruction is rejected
through manifest SHA-256 checks, payload HMAC record binding, or AEAD tag failure.

This test is the ChaCha counterpart of the chaos tamper test. It is intended for
framework_proxy_payload_matched_chachapoly_optimized.py.
"""
...
def make_payload_case(
    ...
    """
    Create a tampered payload while keeping the original manifest binding.

    A payload-tamper test should prove that changing payload bytes/metadata is
    rejected by the protected package. Therefore the manifest's payload_sha256
    must stay as the original value.

    If we recompute payload_sha256 here, we are simulating an attacker who can
    also rewrite the package manifest. That bypasses the manifest-hash layer and
    turns the test into a different AEAD-only diagnostic.
    """
    ...
    # Point the manifest to the tampered payload file, but keep the original
    # payload_sha256. This makes every payload mutation fail package verification.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="ChaCha20-Poly1305 AEAD payload tamper/manifest verification tests.")
    ...
    payload_mutators: Dict[str, Callable[[List[dict]], List[dict]]] = {
        ...
        "poly1305_tag_flip": lambda recs: mutate_first_roi(recs, lambda frame, roi: roi.__setitem__("tag", flip_hex_char(str(roi.get("tag", ""))))),
        "aead_nonce_flip": lambda recs: mutate_first_roi(recs, lambda frame, roi: roi.setdefault("crypto_meta", {}).__setitem__("stream_nonce_b64", flip_b64_byte(str(roi.get("crypto_meta", {}).get("stream_nonce_b64", ""))))),
        "aead_tag_field_flip": lambda recs: mutate_first_roi(recs, lambda frame, roi: roi.setdefault("crypto_meta", {}).__setitem__("tags_b64", flip_b64_byte(str(roi.get("crypto_meta", {}).get("tags_b64", ""))))),
    ...
    for name, payload_path, manifest_path, expected_reject in cases:
        ...
        results.append({
            ...
            "note": "If this passes unexpectedly, the current ChaCha payload may not bind that field strongly enough or may skip reconstruction silently.",
        })

    report = {
        "focus": "ChaCha20-Poly1305 payload tamper response: manifest SHA-256 binding + AEAD/HMAC-bound payload metadata/ciphertext.",
