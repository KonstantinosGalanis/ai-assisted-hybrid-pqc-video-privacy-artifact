...
def main():
    ap = argparse.ArgumentParser(
        description="Dedicated sweep for correlation analysis over different key sizes and frame classes for the ChaCha-based framework."
    )
    ...
    ap.add_argument("--video_preview_mode", default="chacha", choices=["chacha","mosaic","blur","black"], help="preview mode used when the framework path is a proxy/payload framework")
    ap.add_argument("--correlation_script", default="03_correlation_tests_roi.py")
    ap.add_argument("--out", required=True)
    ap.add_argument("--master_key", required=True)
    ap.add_argument("--frame_classes", default="I,P,B", help="comma-separated frame classes to sweep, e.g. I,P,B")
    ap.add_argument("--maps", default=None, help=argparse.SUPPRESS)
    ...
    rows = []
    raw_classes = args.frame_classes if args.frame_classes else (args.maps or "I,P,B")
    frame_classes = [m.strip() for m in raw_classes.split(",") if m.strip()]
    ...
    for frame_class in frame_classes:
        for key_bytes in key_sizes:
            ...
            cipher_frames = encrypt_frames_with_sidecar(frames, recs, ff, derived_key, force_frame_class=frame_class, preview_mode=args.video_preview_mode)
            out_cipher = write_media(cipher_frames, str(generated_dir / f"{frame_class}_k{key_bytes}.mkv"), fps=fps)
