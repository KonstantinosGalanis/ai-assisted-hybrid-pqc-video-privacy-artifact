...
"""
17_mask_expansion_ablation.py

Chaos-first privacy/utility ablation for mask expansion / silhouette leakage.
It rewrites an existing ROI sidecar with padding and mask dilation, reuses those
ROIs for chaos/proxy encryption, then runs detector leakage + quality + storage
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Mask expansion / silhouette leakage ablation for chaos ROI protection.")
    ...
    ap.add_argument("--preview_modes", default="chaos")
    ...
    for pad, dil, mode in cfgs:
        ...
        if args.framework_base_path:
            # Payload/proxy framework: preview mode is configurable and original ROI pixels are stored in payload.
            cmd.extend(["--base_framework", args.framework_base_path, "--payload", str(payload_out), "--manifest", str(manifest_out), "--video_preview_mode", mode, "--audio_preview_mode", "chaos"])
        else:
            # Base chaos framework has no public preview-mode CLI; it always performs chaos pixel substitution.
    ...
    report = {"focus": "Mask expansion / silhouette leakage ablation", "configs_requested": len(cfgs), "rows": rows}
