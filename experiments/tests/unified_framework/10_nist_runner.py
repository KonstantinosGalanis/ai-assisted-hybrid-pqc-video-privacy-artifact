...
def _extract_cipher_payload_from_jsonl_dump(dump_path: str, out_dir: Path) -> Dict[str, str]:
    """Extract raw cipher payload bytes from a JSONL dump.

    Supported record field names (base64-encoded):
      - cipher_payload_b64
      - ciphertext_b64
      - encrypted_payload_b64
      - payload_b64

    This runner intentionally does *not* try to derive payload bytes from the
    PSD transform debug material alone. If the dump only contains permutation /
    sbox / diffusion debug state, that is not the cipher payload and should not
    be fed to NIST as ciphertext.
    """
    supported_fields = (
        "cipher_payload_b64",
        "ciphertext_b64",
        ...
        for lineno, line in enumerate(f, start=1):
            ...
            for field in supported_fields:
                ...
                if isinstance(b64, str) and b64:
                    try:
                        payload_parts.append(base64.b64decode(b64, validate=True))
                    except Exception as e:
                        raise SystemExit(f"Bad base64 in field '{field}' at line {lineno}: {e}") from e
                    found = True
                    seen_any_payload = True
                    break
            if not found and any(k in rec for k in supported_fields):
                raise SystemExit(f"Record at line {lineno} has a payload field but it is empty or invalid.")

    if not seen_any_payload:
        if saw_psd_debug_records:
            raise SystemExit(
                "The provided dump is a PSD transform debug dump, but it does not contain raw cipher payload bytes. "
                "Use --cipher for a ciphertext file, --cipher_payload for a raw payload dump, or extend the framework "
                "to emit a base64 field such as 'cipher_payload_b64'."
            )
        raise SystemExit(
            "No cipher payload field found in the JSONL dump. Supported base64 fields are: "
    ...
    return {
        ...
        "notes": "Extracted concatenated cipher payload bytes from JSONL dump.",
    }
...
def resolve_bytes(args) -> Dict[str, str]:
    ...
    if args.keystream:
        return {
            "source": "legacy_keystream_bytes",
            "path": args.keystream,
            "derived_path": args.keystream,
            "notes": "Legacy mode for raw map-stream / keystream bytes only; not the preferred metric for the PSD design.",
        }
    if args.roi_sidecar and args.master_key and args.framework_faster_path:
        ...
        return {
            'source': 'legacy_derived_keystream_from_sidecar',
            'path': out_path,
            'derived_path': out_path,
            'notes': 'Legacy compatibility path. For the PSD design, prefer --cipher_payload or --cipher.',
        }
    raise SystemExit(
        'Need one of: --cipher_payload, --cipher_payload_dump, --cipher, --bitstream, '
        '--keystream (legacy), or (--roi_sidecar + --master_key + --framework_faster_path) (legacy).'
    )
...
def main():
    ap = argparse.ArgumentParser(
        description='NIST SP800-22 runner. Preferred input for the PSD framework is encrypted payload bytes, not legacy keystream bytes.'
    )
    ap.add_argument('--out', required=True)
    ap.add_argument('--cipher_payload', default=None, help='raw encrypted payload bytes (preferred for the PSD design)')
    ap.add_argument('--cipher_payload_dump', default=None, help='JSONL dump that contains base64 cipher payload fields such as cipher_payload_b64')
    ...
    ap.add_argument('--keystream', default=None, help='legacy raw keystream / map-stream bytes')
    ap.add_argument('--roi_sidecar', default=None, help='legacy compatibility: derive raw keystream from ROI sidecar')
