...
def run_framework_encrypt_subprocess(
    ...
    cmd.extend(["--video_preview_mode", "chaos", "--audio_preview_mode", "chaos"])
...
def _make_keystream(ff, seed: bytes, map_name: str, n: int, burn: int):
    if map_name == "chen" and hasattr(ff, "chen_keystream_u8"):
        return ff.chen_keystream_u8(seed, n, burn=burn)
    if map_name == "cubic" and hasattr(ff, "cubic_keystream_u8"):
        return ff.cubic_keystream_u8(seed, n, burn=burn)
    if map_name == "skew_tent" and hasattr(ff, "skew_tent_keystream_u8"):
        return ff.skew_tent_keystream_u8(seed, n, burn=burn)
    if hasattr(ff, "keystream_u8"):
        # fallback: no explicit burn sweep available
        return ff.keystream_u8(seed, map_name, n)
    raise RuntimeError("framework_faster.py does not expose usable keystream generators")
...
def hamming_curve(
    ...
    if not hasattr(ff, "derive_seed"):
        return {"note": "framework_faster does not expose derive_seed(...)."}

    seed1 = ff.derive_seed(master_key, 0, 0, 0, map_name)
    seed2 = ff.derive_seed(master_key + "|1", 0, 0, 0, map_name)

    curve = []
    for x in x_values:
        if x_axis == "length":
            ks1 = _make_keystream(ff, seed1, map_name, n=int(x), burn=64).tobytes()
            ks2 = _make_keystream(ff, seed2, map_name, n=int(x), burn=64).tobytes()
        elif x_axis == "burn":
            ks1 = _make_keystream(ff, seed1, map_name, n=int(probe_bytes), burn=int(x)).tobytes()
            ks2 = _make_keystream(ff, seed2, map_name, n=int(probe_bytes), burn=int(x)).tobytes()
        else:
...
def main():
    ...
    ap.add_argument("--ham_map", default="chen", choices=["chen", "cubic", "skew_tent"])
