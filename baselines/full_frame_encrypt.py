...
def derive_master_key_from_password(master_key: str) -> bytes:
    """Derive a 32-byte AES-256 key from a password-like string."""
...
def aes256_ctr_keystream_bytes(length: int, key32: bytes, iv16: bytes) -> bytes:
    """Return the raw AES-CTR keystream for NIST/statistical testing."""
    if length <= 0:
        return b""
    zeros = b"\x00" * int(length)
    return aes256_ctr_crypt_bytes(zeros, key32, iv16)
...
def open_lossless_writer(out_path: str, width: int, height: int, fps: float):
    out = av.open(out_path, mode="w", format="matroska")
    rate = _safe_rate(fps)
...
def process_video_full_aes_ctr(
    ...
    # CTR decrypt == encrypt with the same key/IV
    ...
    try:
        ...
        try:
            ...
            if mode == "encrypt" and keystream_dump_path:
                ...
                print(f"[INFO] Keystream dump enabled: {keystream_dump_path}")
            ...
            try:
                for av_frame in in_container.decode(in_stream):
                    ...
                    if ks_f is not None:
                        video_keystream = aes256_ctr_keystream_bytes(frame.size, video_key, video_iv)
                        ks_f.write(video_keystream)
                    ...
                    if has_audio:
                        ...
                        if end > start:
                            ...
                            if ks_f is not None:
                                audio_keystream = aes256_ctr_keystream_bytes(seg.view(np.uint8).size, audio_key, audio_iv)
                                ks_f.write(audio_keystream)
...
# -----------------------------
# Convenience wrapper for password input
# -----------------------------
...
if __name__ == "__main__":
    ...
    ap.add_argument("--in", dest="inp", required=True, help="input video path (.mkv recommended)")
    ap.add_argument("--out", required=True, help="output video path (.mkv recommended)")
    ...
    ap.add_argument("--cipher_dump", default=None, help="(encrypt only) optional path to dump ciphertext bytes")
    ap.add_argument("--keystream_dump", default=None, help="(encrypt only) optional path to dump raw AES-CTR keystream bytes for NIST testing")
    args = ap.parse_args()
    ...
    print("[INFO] Container: MKV (Matroska)")
